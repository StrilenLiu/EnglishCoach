...
1. **先讨论，后动手。没有明确的开工指令，不碰任何文件。**
   收到需求先调研代码、给方案、讲清楚要改哪些文件和为什么，**然后停下来等一句
   明确的开工指令**。这是本仓库的硬规矩，优先于其他所有条目。

   | | |
   |---|---|
   | **随便做，不用问** | 读代码、grep、跑只读命令、分析问题、给方案和建议 |
   | **必须先拿到同意** | 新建 / 修改 / 删除任何文件、commit、push、合并分支 |

   「你觉得呢」「有什么建议」「能不能做到」「帮我看看」都是**征询意见**，
   不是开工令。判断不了算不算同意，就问一句，别猜。

   一次同意只对当次改动有效，不延续到下一件事。

2. **改动先落工作分支，验证通过才合并 main。**
   `main` 永远保持可编译、可发版的状态。新功能与 bug 修复一律推到当前工作
   分支，等作者在本机验证过（界面、三平台编译、真实产物）并明确说合并，
   才动 `main`。

   云端只能验证语法、逻辑和文档同步——界面长什么样、产物能不能跑，看不见。
   所以「我这边全绿」不等于可以合并，合并权始终在作者手上。

3. **v2.15.12 是里程碑版本，当前运行良好。非必要不改动良好的部分。**
   改动前先说清楚会碰哪里、不碰哪里；改完验证已有功能没受影响。

4. **mac 相关代码一律不动。** mac 的深浅主题、下拉、滚动条经过长期打磨已经完美，
   踩过很多坑才到今天。所有主题相关函数的 `if sys.platform == "darwin":` 分支
   都在前面 `return`，只改非 mac 分支。改完要伪装 darwin 验证 mac 路径原样执行。

5. **回答简洁。** 不写万字长文、不写学术论文式的说明。
